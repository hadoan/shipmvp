// import checkLicense from "@shipmvp/features/ee/common/server/checkLicense";
// import ImpersonationProvider from "@shipmvp/features/ee/impersonation/lib/ImpersonationProvider";
// import { clientSecretVerifier, hostedCal, isSAMLLoginEnabled } from "@shipmvp/features/ee/sso/lib/saml";
...
import { defaultCookies } from "@shipmvp/lib/default-cookies";
import { isENVDev } from "@shipmvp/lib/env";
import { randomString } from "@shipmvp/lib/random";
...
import { teamMetadataSchema, userMetadata } from "@shipmvp/prisma/zod-utils";
...
const providers: Provider[] = [
  CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      if (user.identityProvider !== IdentityProvider.TEK && !credentials.totpCode) {
        throw new Error(ErrorCode.ThirdPartyIdentityProviderEnabled);
      }

      if (!user.password && user.identityProvider !== IdentityProvider.TEK && !credentials.totpCode) {
        throw new Error(ErrorCode.IncorrectUsernamePassword);
      }
      ...
      const validateRole = (role: UserPermissionRole) => {
        ...
        // User's identity provider is not "TEK"
        if (user.identityProvider !== IdentityProvider.TEK) return role;
...
const shipmvpAdapter = MyAppAuthAdapter(prisma);
    ...
    async signIn(params) {
      ...
      if (account?.provider) {
          ...
          if (existingUser.email === user.email) {
            try {
              ...
              if (existingUser.accounts.length === 0) {
                ...
                await shipmvpAdapter.linkAccount(linkAccountWithUserData);
              }
            } catch (error) {
        ...
        if (existingUserWithEmail) {
          ...
          if (
            existingUserWithEmail.identityProvider === IdentityProvider.TEK &&
          ...
          } else if (existingUserWithEmail.identityProvider === IdentityProvider.TEK) {
        ...
        await shipmvpAdapter.linkAccount(linkAccountNewUserData);
