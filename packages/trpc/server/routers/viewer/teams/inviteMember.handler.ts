...
import { sendOrganizationAutoJoinEmail, sendTeamInviteEmail } from "@shipmvp/emails";
import { WEBAPP_URL } from "@shipmvp/lib/constants";
import { getTranslation } from "@shipmvp/lib/server/i18n";
import { isTeamAdmin } from "@shipmvp/lib/server/queries";
import { isOrganisationAdmin } from "@shipmvp/lib/server/queries/organisations";
import { prisma } from "@shipmvp/prisma";
import type { Team, User } from "@shipmvp/prisma/client";
import type { MembershipRole } from "@shipmvp/prisma/enums";
  ...
  for (const usernameOrEmail of emailsToInvite) {
    ...
    if (!invitee) {
      ...
      if (input.sendEmailInvitation && ctx?.user?.name && team?.name) {
         ...
         * This only changes if the user is a TEK user and has not completed onboarding and has no password
         */
        if (!invitee.completedOnboarding && !invitee.password && invitee.identityProvider === "TEK") {
