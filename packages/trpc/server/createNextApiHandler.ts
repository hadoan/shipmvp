...
import * as trpcNext from "@shipmvp/trpc/server/adapters/next";
import { createContext as createTrpcContext } from "@shipmvp/trpc/server/createContext";
...
export function createNextApiHandler(router: AnyRouter, isPublic = false) {
  return trpcNext.createNextApiHandler({
    ...
    responseMeta({ ctx, paths, type, errors }) {
      ...
      if (timezone.success) defaultHeaders.headers["x-shipmvp-timezone"] = timezone.data;
      ...
      if (isPublic && paths) {
        ...
        const cacheRules = {
          session: `no-cache`,
          i18n: `no-cache`,
          // Revalidation time here should be 1 second, per https://github.com/shipmvp/shipmvp.co/pull/6823#issuecomment-1423215321
